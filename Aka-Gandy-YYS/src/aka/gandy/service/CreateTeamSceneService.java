...
public class CreateTeamSceneService implements SceneService{
	private Scene createTeamScene = SceneFactory.getCreateTeamScene();
	private String name = "CreateScene";
	private int mode;
	@Override
	public boolean isMatch(IplImage baseImage) {
		for (String template : createTeamScene.getKeyTemplate()) {
	...
	public boolean click(int hwnd, IplImage baseImage) {
		...
		for(String tempPath : createTeamScene.getClickMap().get(mode)) {

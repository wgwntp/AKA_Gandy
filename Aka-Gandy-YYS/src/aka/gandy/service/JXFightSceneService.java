...
public class JXFightSceneService implements SceneService{
	private Scene jxFightScene = SceneFactory.getJXFightScene();
	private String name = "JXFightScene";
	private int mode;
	@Override
	public boolean isMatch(IplImage baseImage) {
		for (String template : jxFightScene.getKeyTemplate()) {
	...
	public boolean click(int hwnd, IplImage baseImage) {
		...
		for(String tempPath : jxFightScene.getClickMap().get(mode)) {

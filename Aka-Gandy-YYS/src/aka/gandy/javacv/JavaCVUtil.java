...
public class JavaCVUtil {
	...
	public static void imgMatch(String[] args) {
		...
		IplImage tempIpl = cvLoadImage(args[1]);
		Mat sourceTemp = cvarrToMat(tempIpl);
		Mat template = new Mat(sourceTemp.size(), CV_8UC1);
		cvtColor(sourceTemp, template, COLOR_BGR2GRAY);
		...
		Mat tempMat = sourceColor.apply(new Rect(max.x(), max.y(), template.cols(), template.rows()));
		IplImage matImage = new IplImage(tempMat);
		CvHistogram tempHis = getHueHistogram(cvLoadImage(args[1]));
		double point = cvCompareHist(tempHis, JavaCVUtil.getHueHistogram(matImage), CV_COMP_INTERSECT);
	...
	private static void testLoad() {
		imshow("t",imread("template/t_main_fy.PNG", CV_LOAD_IMAGE_GRAYSCALE));
		waitKey(0);
	}
	...
	public static CvHistogram getGrayImageHistogram(IplImage greyImage) {
		if (greyImage == null || greyImage.nChannels() < 1)
			new Exception("Error!");
...
//		greyImage = null;
	...
	public static void release(IplImage baseImage) {
		baseImage.release();
		cvReleaseImage(baseImage);
		baseImage = null;
	}
	public static void main(String[] args) {
		...
		imgMatch(new String[]{"UIType/yhsl.png", "template/t_sl.png"});
